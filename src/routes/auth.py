...
async def forgot_password(email: EmailStr, background_tasks: BackgroundTasks, request: Request):
    user = await repositories_users.get_user_by_email(email)
    if user:
        token = auth_service.create_email_token({"sub": email})
        background_tasks.add_task(send_reset_password_email, email, token, request.base_url)
    return JSONResponse(content={"message": "If the email exists, a reset password email has been sent."})


async def send_reset_password_email(email: EmailStr, token: str, base_url: str):
    # Implementation to send reset password email with token
    pass